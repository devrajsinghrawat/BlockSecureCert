...
function setUpParams() {
  ...
  if (process.env.NODE_ENV === 'local') {
    port = process.env.PORT_BE_LOCAL;
  } else {
    port = process.env.NODE_ENV === 'staging'
    ? process.env.PORT_BE_STG
    : process.env.PORT_BE_PRD;
  }  


    console.log("port", process.env.NODE_ENV , process.env.PORT_BE_DEV );
